...
class _AddProjectState extends State<AddProject> {
  ...
  Future<void> fetchUploadedFiles() async {
    if (widget.projectID.isNotEmpty) {
      ...
      setState(() {
        uploadedFilesData = files.docs.map((doc) => doc.data()).toList();
        print(uploadedFilesData);
      });
    }
  }
  ...
  Future<void> addProject() async {
    ...
    try {
      if (selectedFiles.isNotEmpty) {
        ...
        for (var file in selectedFiles) {
          ...
          final storageRef = FirebaseStorage.instance
              .ref()
              .child('Project/${projectID.toString()}/${file.name}');
          final uploadTask = storageRef.putFile(File(file.path!));
          await uploadTask.whenComplete(() async {
      ...
      showSnackBar(context: context, message: 'Failed to save files: $e');
    }
  }
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        if (projectData != null) {
          ...
          // Upload new files and update their details
          await uploadFilesAndUpdateProject(projectSnapshot.reference);
        } else {
          throw ('Document data is null or empty');
        }
      } else {
  ...
  Future<void> uploadFilesAndUpdateProject(DocumentReference projectRef) async {
    if (selectedFiles.isNotEmpty) {
      for (var file in selectedFiles) {
        final storageRef = FirebaseStorage.instance
            .ref()
            .child('Project/${widget.projectID}/${file.name}');
        final uploadTask = storageRef.putFile(File(file.path!));
        await uploadTask.whenComplete(() async {
          final downloadUrl = await storageRef.getDownloadURL();
          // Store or update file metadata in Firestore
          final existingFile = await projectRef
              .collection('projects')
              .doc(widget.projectID)
              .collection('documents')
              .where('documentName', isEqualTo: file.name)
              .limit(1)
              .get();
          if (existingFile.docs.isNotEmpty) {
            // If file already exists, update its details
            await existingFile.docs.first.reference.update({
              'documentPath': downloadUrl,
            });
          } else {
            // If file doesn't exist, add a new document entry
            await FirebaseFirestore.instance
                .collection('projects')
                .doc(widget.projectID)
                .collection('documents')
  ...
  void saveProjectFiles() async {}

// Function to generate a 6-digit document ID
...
// Function to determine document type based on file extension
  String getDocumentType(String fileName) {
    // Split file name by dot to get extension
    ...
    return 'Other'; // Return 'Other' if no extension found
  }
                              ...
                              return TableRow(
                                children: [
                                  TableCell(
                                    child: Padding(
                                      padding: const EdgeInsets.all(8.0),
                                      child: Text(fileData['documentName']),
                                    ),
                                  ),
                                  TableCell(
                                      child: IconButton(
                                    ...
                                    onPressed: () async {
                                      // Implement a function to delete the file from Firebase Storage and Firestore
                                      await deleteUploadedFile(fileData);
                                    },
                                  )),
                                ],
                              );
                                ...
                                onPressed: () {
                                  setState(() {
                                    selectedFiles.removeAt(
                                        index); // Remove the file from the selectedFiles list
                                  });
                                },
                              )),
                            ]);
  ...
  Future<void> deleteUploadedFile(Map<String, dynamic> fileData) async {
    print("called");
    try {
      ...
      final storageRef = FirebaseStorage.instance
          .ref()
          .child('Project/${widget.projectID}/' + fileData['documentName']);
      ...
      // Remove file entry from uploadedFilesData list
      setState(() {
        uploadedFilesData.remove(fileData);
      });

      // Show snackbar indicating successful deletion
      showSnackBar(context: context, message: "File deleted successfully");
    } catch (e) {
      // Handle errors
      print(e);
      showSnackBar(context: context, message: "Error deleting file: $e");
    }
  }
}
